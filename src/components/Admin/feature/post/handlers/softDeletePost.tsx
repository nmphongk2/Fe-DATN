...
import { getListPostThunk, softDeletePostThunk } from "../../../../../redux/post/thunk";

const MySwal = withReactContent(Swal);

export const handlesoftDeletePost = (
  postId: string, 
  ...
  MySwal.fire({
    ...
    if (result.isConfirmed) {
      try {
        const action = await dispatch(softDeletePostThunk(postId));
        if (softDeletePostThunk.rejected.match(action)) {
