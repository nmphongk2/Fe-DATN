...
import { updateAuctionThunk, getOneAuctionThunk } from "../../../../redux/product/admin/Thunk";
...
const EditProduct: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setIsLoading(true);
      try {
        await dispatch(getOneAuctionThunk(id)).unwrap();
      } catch (error) {
  ...
  const submitFormEdit: SubmitHandler<ProductAuction> = async (data) => {
    if (!id) {
    ...
    setIsLoading(true);
    try {
      const actionResult = await dispatch(updateAuctionThunk({ id, product: data })).unwrap();
  ...
  return (
    <form onSubmit={handleSubmit(submitFormEdit)} encType="multipart/form-data">
      <ToastContainer />
      <ReusableBreadcrumb items={breadcrumbItems.editProductAuction} />
      <div className="mb-4 ml-4 col-span-full xl:mb-2">
        <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">
          Cập nhật {product?.product_name}
        </h1>
      </div>
