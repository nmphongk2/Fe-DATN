...
const softDeleteListSupplier: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDeleteSuppliers = async () => {
      setLoading(true);
      setIsSearching(true);
      ...
      try {
        ...
        const response = keyword.trim()
          ? await searchDeleteSuppliers(keyword, page)
          : await getSoftDeletedSuppliers(page);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = searchTerm.trim()
        ? await searchDeleteSuppliers(searchTerm.trim(), 1)  // Luôn tìm từ trang 1 khi có từ khóa
        : await getSoftDeletedSuppliers(1);  // Nếu không có từ khóa, trả về trang đầu tiên của danh sách
