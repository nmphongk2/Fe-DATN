    ...
    const fetchDeleteBrands = async () => {
      ...
      try {
        ...
        const response = keyword.trim()
          ? await searchDeleteBrands(keyword, page)
          : await getSoftDeletedBrands(page);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = searchTerm.trim()
        ? await searchDeleteBrands(searchTerm.trim(), 1)  // Luôn tìm từ trang 1 khi có từ khóa
        : await getSoftDeletedBrands(1);  // Nếu không có từ khóa, trả về trang đầu tiên của danh sách
